...
const test = ({ serverResponse, clientRequest, encoding = 'utf8' }) => {
  ...
  testServer.on('request', (req, res) => {
    assert(
      propsContained(clientRequest.headers, req.headers),
      'request headers not passed to proxied server',
    )
  ...
  testServer.on('listening', () => {
    ...
    proxy.eventEmitter.on('listening', function() {
      const proxyRequest = http.request(clientRequest, proxyRes => {
        ...
        proxyRes.on('end', () => {
          assert(
            propsContained(serverResponse.headers, proxyRes.headers),
            'proxied response headers not passed back',
          )
          assert(
            resBody === serverResponse.body,
            'proxied response body is not passed back',
          )
          assert(
            proxyRes.statusCode === serverResponse.statusCode,
            'statusCode is not equal',
          )
          testServer.close()
        })
      })
